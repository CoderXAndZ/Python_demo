...
def create_Excel(rows,columns,url_array):
    ...
    row0 = [u'时间',u'PHP耗时',u'WWW耗时',u'接口提升/倍',u'接口']

    date = datetime.now().strftime('%Y-%m-%d %H.%M.%S')

    # 写表头
    for i in range(0,len(row0)):
        excel_sheet.write(0, i, row0[i], set_style('Times New Roman', 500, True))
    ...
    for i in range(0,len(rows)): # 控制行
        for j in range(0, len(row0)): # 控制列
            if j == 0: # 时间
                ...
                excel_sheet.write(i+1, j, (rows[i] / columns[i]) * 10, set_style())
            else:
                excel_sheet.write(i+1, j, url_array[i], set_style())
        print("\n第%d行的值为："%(i+1),'%s'%date, rows[i], columns[i], (rows[i] / columns[i]) * 10,url_array[i])

    excel.save('/Users/admin/Desktop/接口测试结果统计/统计结果 %s.xls'%date)

# 开始请求数据，每个接口 10 次
def start_request_ten():
    ...
    for i in range(len(url_array)):
        ...
        for j in range(0,10):
            ...
            www_time_arr.append(result_www[1])
            php_time_arr.append(result_php[1])

        www_total = 0.0 # www 10次总时长
        for k in range(0,len(www_time_arr)):
            www_total += www_time_arr[k]
        www_average = www_total / len(www_time_arr) # www 10次平均值

        php_total = 0.0  # php 10次总时长
        for k in range(0, len(php_time_arr)):
            php_total += php_time_arr[k]
        php_average = php_total / len(php_time_arr)  # www 10次平均值
